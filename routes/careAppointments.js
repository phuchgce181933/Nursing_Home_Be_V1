...
// Validate no overlapping appointment for same resident (excludes cancelled)
...
// POST /api/care-appointments — Create appointment
...
// GET /api/care-appointments — List with filters (history)
...
// GET /api/care-appointments/daily — Schedule for a specific day
router.get('/daily', protect, authorize(...STAFF_ROLES), async (req, res) => {
  try {
...
// GET /api/care-appointments/weekly — Schedule for the week containing a date
router.get('/weekly', protect, authorize(...STAFF_ROLES), async (req, res) => {
  try {
    const { date, residentId } = req.query;
    const refDate = date ? new Date(date) : new Date();
    if (isNaN(refDate)) return res.status(400).json({ message: 'Invalid date' });
...
// GET /api/care-appointments/:id — Get single appointment
...
// PUT /api/care-appointments/:id — Update appointment
...
// DELETE /api/care-appointments/:id — Delete appointment
...
// PUT /api/care-appointments/:id/status — Update status
router.put('/:id/status', protect, authorize(...STAFF_ROLES), async (req, res) => {
  try {
...
// PUT /api/care-appointments/:id/assign-doctor — Assign doctor
router.put('/:id/assign-doctor', protect, authorize(...STAFF_ROLES), async (req, res) => {
  try {
...
// PUT /api/care-appointments/:id/assign-nurse — Assign nurse
router.put('/:id/assign-nurse', protect, authorize(...STAFF_ROLES), async (req, res) => {
  try {
...
// POST /api/care-appointments/:id/reminder — Trigger appointment reminder notifications
router.post('/:id/reminder', protect, authorize(...STAFF_ROLES), async (req, res) => {
  try {
    const appointment = await CareAppointment.findById(req.params.id)
