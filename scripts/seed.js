...
//   ...LEGACY_DEV_EMAILS,
// ];
// const SEED_STAFF_CODES    = ['ADM001', 'MGR001', 'DOC001', 'DOC002', 'NUR001', 'NUR002', 'PHA001', 'CAR001', 'CAR002', 'ADM002', 'MGR002', 'NUR003', 'DOC003'];
...
//   Model.findOneAndUpdate(filter, { $set: data }, { upsert: true, returnDocument: 'after', setDefaultsOnInsert: true });

// const upsertUser = (email, data) =>
//   upsert(User, { $or: [{ email }, { username: data.username }] }, data);
...
//   const pwLegacy = await bcrypt.hash('password123', 10);

//   // ── STRUCTURAL: luôn upsert — _id ổn định qua các lần restart ────────────

//   console.log('👤 Upserting users...');
//   const [admin, manager, doctor1, doctor2, nurse1, nurse2, pharmacist, family1, caregiverSeed] = await Promise.all([
//     upsertUser('admin@seed.com',      { fullName: 'Admin System',       username: 'admin_seed',   passwordHash: pw, role: 'admin',      isActive: true }),
//     upsertUser('manager@seed.com',    { fullName: 'Nguyễn Văn Bình',    username: 'manager_seed', passwordHash: pw, role: 'manager',    isActive: true }),
//     upsertUser('doctor1@seed.com',    { fullName: 'BS. Nguyễn Văn An',  username: 'doctor1_seed', passwordHash: pw, role: 'doctor',     phone: '0901000001', isActive: true }),
//     upsertUser('doctor2@seed.com',    { fullName: 'BS. Lê Văn Hùng',    username: 'doctor2_seed', passwordHash: pw, role: 'doctor',     phone: '0901000002', isActive: true }),
//     upsertUser('nurse1@seed.com',     { fullName: 'ĐD. Trần Thị Bích',  username: 'nurse1_seed',  passwordHash: pw, role: 'nurse',      phone: '0902000001', isActive: true }),
//     upsertUser('nurse2@seed.com',     { fullName: 'ĐD. Phạm Văn Cường', username: 'nurse2_seed',  passwordHash: pw, role: 'nurse',      phone: '0902000002', isActive: true }),
//     upsertUser('pharmacist@seed.com', { fullName: 'DS. Hoàng Thị Lan',  username: 'pharma_seed',  passwordHash: pw, role: 'pharmacist', phone: '0903000001', isActive: true }),
//     upsertUser('family1@seed.com',    { fullName: 'Gia đình Nguyễn',    username: 'family1_seed', passwordHash: pw, role: 'family',     isActive: true }),
//     upsertUser('caregiver@seed.com',  { fullName: 'Hộ lý Lê Văn Care',  username: 'caregiver_seed', passwordHash: pw, role: 'caregiver', phone: '0904000001', isActive: true }),
//   ]);

//   const [adminTest, managerTest, caregiverTest, nurseTest, doctorTest] = await Promise.all([
//     upsertUser('admin@test.com',      { fullName: 'Admin Test',         username: 'admin_test',      passwordHash: pwLegacy, role: 'admin',      isActive: true }),
//     upsertUser('manager@test.com',    { fullName: 'Manager Test',       username: 'manager_test',    passwordHash: pwLegacy, role: 'manager',    isActive: true }),
//     upsertUser('caregiver@test.com',  { fullName: 'Caregiver Test',     username: 'caregiver_test',  passwordHash: pwLegacy, role: 'caregiver',  phone: '0904000099', isActive: true }),
//     upsertUser('nurse@test.com',      { fullName: 'Nurse Test',         username: 'nurse_test',      passwordHash: pwLegacy, role: 'nurse',      phone: '0902000099', isActive: true }),
//     upsertUser('doctor@test.com',     { fullName: 'Doctor Test',        username: 'doctor_test',     passwordHash: pwLegacy, role: 'doctor',     phone: '0901000099', isActive: true }),
//   ]);
...
//   const assignedFloor1 = [res1._id, res2._id, res3._id];
//   const assignedFloor2 = [res4._id, res5._id, res6._id];
...
//     upsert(StaffProfile, { staffCode: 'DOC001' }, { userId: doctor1._id,    roleCategory: 'doctor',     specialty: 'Nội khoa tổng quát',  certifications: ['BLS','ACLS'],      responsibleAreaIds: [floor1._id], assignedResidentIds: assignedFloor1 }),
//     upsert(StaffProfile, { staffCode: 'DOC002' }, { userId: doctor2._id,    roleCategory: 'doctor',     specialty: 'Y học gia đình',       certifications: ['BLS'],             responsibleAreaIds: [floor2._id], assignedResidentIds: assignedFloor2 }),
//     upsert(StaffProfile, { staffCode: 'NUR001' }, { userId: nurse1._id,     roleCategory: 'nurse',      specialty: 'Điều dưỡng chăm sóc', certifications: ['BLS','First Aid'], responsibleAreaIds: [floor1._id], assignedResidentIds: assignedFloor1 }),
//     upsert(StaffProfile, { staffCode: 'NUR002' }, { userId: nurse2._id,     roleCategory: 'nurse',      specialty: 'Điều dưỡng chăm sóc', certifications: ['BLS'],             responsibleAreaIds: [floor2._id], assignedResidentIds: assignedFloor2 }),
...
//     upsert(StaffProfile, { staffCode: 'CAR001' }, { userId: caregiverSeed._id, roleCategory: 'caregiver', specialty: 'Hỗ trợ sinh hoạt hằng ngày', certifications: ['First Aid'], responsibleAreaIds: [floor1._id], assignedResidentIds: assignedFloor1 }),
//     upsert(StaffProfile, { staffCode: 'ADM002' }, { userId: adminTest._id, roleCategory: 'admin', specialty: 'Quản trị hệ thống', assignedResidentIds: [] }),
//     upsert(StaffProfile, { staffCode: 'MGR002' }, { userId: managerTest._id, roleCategory: 'manager', specialty: 'Quản lý vận hành', assignedResidentIds: [] }),
//     upsert(StaffProfile, { staffCode: 'CAR002' }, { userId: caregiverTest._id, roleCategory: 'caregiver', specialty: 'Hỗ trợ sinh hoạt hằng ngày', certifications: ['First Aid'], responsibleAreaIds: [floor1._id], assignedResidentIds: assignedFloor1 }),
//     upsert(StaffProfile, { staffCode: 'NUR003' }, { userId: nurseTest._id, roleCategory: 'nurse', specialty: 'Điều dưỡng chăm sóc', certifications: ['BLS'], responsibleAreaIds: [floor1._id], assignedResidentIds: assignedFloor1 }),
//     upsert(StaffProfile, { staffCode: 'DOC003' }, { userId: doctorTest._id, roleCategory: 'doctor', specialty: 'Nội khoa tổng quát', certifications: ['BLS'], responsibleAreaIds: [floor1._id], assignedResidentIds: assignedFloor1 }),
//   ]);
...
//   // Prescriptions: schema đã đổi (doctorId, validUntil, items[]) — dùng npm run seed:medication:local
...
//   } else {
//     console.log('   ℹ️  Prescriptions skipped (use npm run seed:medication:local for medication data)');
//   }
...
//   if (force) {
//     console.log('\n--- Tài khoản @seed.com (mật khẩu: Password123!) ---');
//     console.log('Admin      : admin@seed.com');
//     console.log('Manager    : manager@seed.com');
//     console.log('Caregiver  : caregiver@seed.com → RES001–003');
...
//     console.log('\n--- Tài khoản @test.com (mật khẩu: password123) ---');
//     console.log('Admin      : admin@test.com');
//     console.log('Manager    : manager@test.com');
//     console.log('Caregiver  : caregiver@test.com → RES001–003');
