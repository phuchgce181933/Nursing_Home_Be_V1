...
const seed = async ({ force = false } = {}) => {
  ...
  const [admin, manager, doctor1, doctor2, nurse1, nurse2, pharmacist, family1, caregiverSeed] = await Promise.all([
    ...
    upsert(User, { email: 'caregiver@seed.com' },  { fullName: 'Hộ lý Lê Văn Care',  username: 'caregiver_seed', passwordHash: pw, role: 'caregiver', phone: '0904000001', isActive: true }),
  ]);

  const [adminTest, managerTest, caregiverTest, nurseTest, doctorTest] = await Promise.all([
    upsert(User, { email: 'admin@test.com' },      { fullName: 'Admin Test',         username: 'admin_test',      passwordHash: pwLegacy, role: 'admin',      isActive: true }),
    upsert(User, { email: 'manager@test.com' },    { fullName: 'Manager Test',       username: 'manager_test',    passwordHash: pwLegacy, role: 'manager',    isActive: true }),
    upsert(User, { email: 'caregiver@test.com' },  { fullName: 'Caregiver Test',     username: 'caregiver_test',  passwordHash: pwLegacy, role: 'caregiver',  phone: '0904000099', isActive: true }),
    upsert(User, { email: 'nurse@test.com' },      { fullName: 'Nurse Test',         username: 'nurse_test',      passwordHash: pwLegacy, role: 'nurse',      phone: '0902000099', isActive: true }),
    upsert(User, { email: 'doctor@test.com' },     { fullName: 'Doctor Test',        username: 'doctor_test',     passwordHash: pwLegacy, role: 'doctor',     phone: '0901000099', isActive: true }),
  ]);
